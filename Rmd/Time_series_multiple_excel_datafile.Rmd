---
title: "Plotting Time Series Data from several files"
...
```{r}
here::i_am("Preliminary_PlotTwist_like.R")
```
...
```{r}
data_path <- paste(here("data"), list.files(here("data")), sep = "/")
head(data_path)
```
...
```{r}
names(df_input) <- str_replace(list.files(here("data")), pattern = ".xlsx", replacement = "")
head(df_input)
```
...
2.  Give the generic name "Time" to the column "Time (sec)".\
    The function"rename" is used to change the name of the column in the following way: "new_name=old_name".
    **This might be changed with a user input command to choose the desired name**
